...
glm_model8<- glmnet(x[glmnet_train,], y[glmnet_train], alpha=1, lambda = grid)
plot(glm_model8, xvar="lambda")

##BEST LAMBDA IS $lambda.min
set.seed(2)
cvlasso<- cv.glmnet(x[glmnet_train,], y[glmnet_train], alpha=1)
cvlasso
...
model8 <- mlogit(Choice~CC+GN+NS+BU+FA+LD+BZ+FC+FP+RP+PP+KA+SC+TS+NV+MA+LB+AF+HU+Price -1 |year+miles+region+segment, data = D2)
