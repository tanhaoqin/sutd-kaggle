...
for (i in 1:6){
  ...
  cvmodel2Vector[[i]] <- mlogit(Choice~GN+NS+BU+FA+LD+BZ+FC+FP+RP+PP+KA+SC+TS+NV+MA+LB+AF+HU+Price -1| gender+miles+night+age+Urb, data = trainVector[[i]])
  
  cvmodel3Vector[[i]] <- mlogit(Choice~CC+GN+NS+FA+LD+BZ+FC+FP+RP+PP+KA+SC+TS+NV+MA+LB+AF+HU+Price -1| gender+miles+night+age+Urb, data = trainVector[[i]])
  
  cvmodel4Vector[[i]] <- mlogit(Choice~CC+GN+NS+BU+FA+LD+BZ+FC+FP+RP+PP+SC+TS+NV+MA+LB+AF+HU+Price -1| gender+miles+night+age+Urb, data = trainVector[[i]])
...
Pred11_male <- predict(model7_pre_male, newdata = V2)
Pred11_female <- predict(model7_pre, newdata = V2)
length(Pred11_female)
Pred11_male[1,2]

Pred11_tgt <- Pred11_male

for(i in 1:nrow(Pred11_male)){
  for (j in 1:4){
    Pred11_tgt[i,j] <- (Pred11_male[i,j]+Pred11_female[i,j])/2
  }
}
...
prediction_score(ActualChoice2, Pred11_male)
prediction_score(ActualChoice2female, Pred11_female)
```
...
Pred11_old <- predict(model7_pre_old, newdata = V2)
Pred11_young <- predict(model7_pre, newdata = V2)
length(Pred11_young)
Pred11_old[1,2]

Pred11_allages <- Pred11_old

for(i in 1:nrow(Pred11_old)){
  for (j in 1:4){
    Pred11_allages[i,j] <- (Pred11_old[i,j]+Pred11_young[i,j])/2
  }
}
...
prediction_score(ActualChoice, Pred11_old)
prediction_score(ActualChoice, Pred11_young)
```
...
Pred11_highincome <- predict(model7_pre_highincome, newdata = V2)
Pred11_lowincome <- predict(model7_pre_lowincome, newdata = V2)

length(Pred11_lowincome)
Pred11_highincome[1,2]

Pred11_allincome <- Pred11_highincome

for(i in 1:nrow(Pred11_highincome)){
  for (j in 1:4){
    Pred11_allincome[i,j] <- (Pred11_highincome[i,j]+Pred11_lowincome[i,j])/2
  }
}
...
prediction_score(ActualChoice, Pred11_highincome)
...
Pred11_fullsize <- predict(model7_pre_fullsize, newdata = V2)
Pred11_midsize_car <- predict(model7_pre_midsize_car, newdata = V2)
Pred11_midsize_utility <- predict(model7_pre_midsize_utility, newdata = V2)
Pred11_luxury_utilty <- predict(model7_pre_luxury_utility, newdata = V2)
Pred11_luxury_sedan <- predict(model7_pre_luxury_sedan, newdata = V2)
Pred11_small_car <- predict(model7_pre_small_car, newdata = V2)



Pred11_allsegments <- Pred11_fullsize

for(i in 1:nrow(Pred11_fullsize)){
  for (j in 1:4){
    Pred11_allsegments[i,j] <- (Pred11_fullsize[i,j]+Pred11_midsize_car[i,j]+Pred11_midsize_utility[i,j]+Pred11_luxury_utilty[i,j]+Pred11_luxury_sedan[i,j]+Pred11_small_car[i,j])/6
  }
}
...
prediction_score(ActualChoice, Pred11_fullsize)
prediction_score(ActualChoice, Pred11_midsize_car)
prediction_score(ActualChoice, Pred11_midsize_utility)
prediction_score(ActualChoice, Pred11_luxury_utilty)
prediction_score(ActualChoice, Pred11_luxury_sedan)
prediction_score(ActualChoice, Pred11_small_car)
...
ClusterTrainData <- train[train$Task == 1,c(84,86:89,91:92)]
fac <- factor(c())
levels <- c()
...
```{r pre-processing data}
ClusterTrainData <- train2[train2$Task == 1,c(84,86:94)]
fac <- factor(c())
for(i in c('segment', 'region')){
  #print(levels(ClusterTrainData[,i]))
